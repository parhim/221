...
public:
    ...
    void progressTime();
    /*--------------
     A function to increment the time of every element in the queue
     ------*/
    ...
    /*--------------
     A function to search through a queue for a particular key value, and if found, move the node to the front of the same queue.
     ------*/
    void merge_two_queues(Queue::Queue q2);
    /*
     A function that takes two queues that are each ordered according to a given field and merges the two queues into one queue whose absolute order of the merged elements is kept.
     */
...
private:
    ...
    public:
        ...
        /*-------------------------------------------------------------------
         ...
         ------------------------------------------------------------------*/
        { data = value;timeInQueue = 0;next = link; }
